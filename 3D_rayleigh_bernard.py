...
problem.add_equation("dt(u) + dx(p) - (dx(dx(u)) + dy(dy(u)) + dz(uz)) - 2 * v * (1 / Ek) * sin(phi) = - (u * dx(u) + v * dy(u) + w * uz)")
...
problem.add_equation("dt(w) + dz(p) - (dx(dx(w)) + dy(dy(w)) + dz(wz)) + 2 * v * (1 / Ek) * cos(phi) - X * T = -(u * dx(w) + v * dy(w) + w * wz)")
